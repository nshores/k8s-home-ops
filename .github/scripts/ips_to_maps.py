...
# ## EDIT THESE SETTINGS ##
TAUTULLI_APIKEY = '77bb9f9c14834eca924976b2a15f1929'  # Your Tautulli API key
TAUTULLI_URL = 'https://tautulli.lab.nickshores.net'  # Your Tautulli URL
...
LAN_SUBNET = ('10.10', '127.0.0', '192.168.99.0')
REPLACEMENT_WAN_IP = '73.48.249.10'

# Enter Friendly name for Server ie 'John Smith'
SERVER_FRIENDLY = 'NickFlix'
...
def draw_map(map_type, geo_dict, filename, headless, leg_choice):
    ...
    if map_type == 'NA':
        ...
        proj = ccrs.LambertConformal(central_longitude=35, central_latitude=45)
        extent = [-15, 75, 20, 70]
    elif map_type == 'World':
        proj = ccrs.Robinson()
    ...
    # Map base features
    ax.add_feature(cfeature.LAND.with_scale('50m'), facecolor='#3C3C3C')
    ax.add_feature(cfeature.OCEAN.with_scale('50m'), facecolor='#1F1F1F')
    ...
    for key, values in geo_dict.items():
        for data in values:
            try:
                ...
                if ip.startswith(LAN_SUBNET) and not REPLACEMENT_WAN_IP:
                    print(f"Skipping LAN IP: {ip}")
                    continue

                color = PLATFORM_COLORS.get(platform, DEFAULT_COLOR)
                marker = '*' if key == SERVER_FRIENDLY else '.'
                zorder = 3 if key == SERVER_FRIENDLY else 2
                alpha = 1 if key == SERVER_FRIENDLY else 0.4
                size = 80 if key == SERVER_FRIENDLY else max(10, play_count)

                label = f"Location: {data['city']}, {data['region']},  User: {key}\nPlatform: {platform}, IP: {ip}, Play Count: {play_count}"
                point = ax.plot(lon, lat, marker=marker, color=color, markersize=size / 10,
                                transform=ccrs.PlateCarree(), alpha=alpha, zorder=zorder, label=label)

                if label not in labels:
                    handles.append(point[0])
                    labels.append(label)

                if (lon != server_lon) and (lat != server_lat):
                    ax.plot([lon, server_lon], [lat, server_lat], color=color,
                            transform=ccrs.Geodetic(), linewidth=1.5, alpha=0.6)

            except Exception as e:
                print(f"Error plotting user {key}: {e}")

    if leg_choice and labels:
        leg = plt.legend(handles, labels, fancybox=True, fontsize='x-small',
                         loc='upper left', title="Legend", labelspacing=1.0, borderpad=1.5, handletextpad=2.)
        leg.get_title().set_color('#7B777C')
        leg.get_frame().set_facecolor('#2C2C2C')
        for text in leg.get_texts():
            plt.setp(text, color='#A5A5A7')

    plt.title(title_string, fontsize=16, color='white')
    ...
    for key, values in geo_dict.items():
        for data in values:
            try:
                ...
                # Plot user point
                ax.plot(lon, lat, marker=marker, color=color, markersize=size / 10,
                        transform=ccrs.PlateCarree(), alpha=alpha, zorder=zorder)
    ...
    if leg_choice:
        plt.legend([SERVER_FRIENDLY], loc='upper left', fontsize='small')
    ...
    for key, values in geo_dict.items():
        for data in values:
            try:
                ...
                # Draw line to server
                if (lon != server_lon) and (lat != server_lat):
                    geoline = Geodesic().line([[lon, lat], [server_lon, server_lat]])
                    ax.plot(geoline[:, 0], geoline[:, 1], color=color,
                            transform=ccrs.Geodetic(), linewidth=1.5, alpha=0.6)
